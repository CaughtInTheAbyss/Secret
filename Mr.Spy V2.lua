...
local MrSpyV2Rebug = Instance.new("ScreenGui")
...
--Properties:
MrSpyV2Rebug.Name = "Mr.Spy V2 - Rebug"
MrSpyV2Rebug.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
MrSpyV2Rebug.ResetOnSpawn = false

Hub.Name = "Hub"
Hub.Parent = MrSpyV2Rebug
...
getfenv().script = Instance.new('LocalScript', MrSpyV2Rebug)
...
function LoadR2S(obj,args)
	...
	local String = ""
	for i in string.gmatch(GetFullName(obj),".") do
		if i == "." then
...
if replaceclosure then
	...
	local Hook = protect(function(self,...)
		...
		if (not IgnoreThis[self.Name]) and SpyingRemotes[self] then print(self, type(self))
	        AddToOutput(self, ReturnArgs(args));
	    end
